 ...
 *
 *调度器的处理流程:
 * 处理请求会作为事件进入系统，由中央异步调度器(AsyncDispatcher)负责传递给相应的事件调度
 * 器(eventHandle).该事件调度器可能将该事件转发给另外一个事件调度器，也可能交给一个带有有限状态机
 * 的事件处理器，其处理结果也以事件的形式输出给中央异步调度器，而新的事件会再次被中央异步调度器转发
 * 给下一个事件的调度器，直至处理结束
