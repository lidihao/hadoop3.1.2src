...
public class BlockPlacementPolicies{
  ...
  private final BlockPlacementPolicy replicationPolicy;// 连续类型
  private final BlockPlacementPolicy ecPolicy; // 条类型

  public BlockPlacementPolicies(Configuration conf, FSClusterStats stats,
                                NetworkTopology clusterMap,
                                Host2NodesMap host2datanodeMap){
    // replicas默认的BlockPlacementPolicies是BlockPlacementPolicyDefault(默认)
    ...
    // EC默认的BlockPlacementPolicies是BlockPlacementPolicyRackFaultTolerant(时副本尽量分布在不同的机架上)
