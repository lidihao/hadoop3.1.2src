 ...
 * 对于块状态管理，如果发生节点退役,nameNdoe切换,数据节点失效，BlockManger维护live replicas ==expected redundancy
 ...
 *
 * 维护模式，live replica不需要等于expected redundancy,维护模式是什么鬼?？？
...
public class BlockManager implements BlockStatsMXBean {
  ...
  // 命名空间，实现类为FSNamesystem
  private final Namesystem namesystem;
  // BlockManager安全模式
  private final BlockManagerSafeMode bmSafeMode;
  // 管理DataNdoe,DataNode的上线与下线或者其他操作
  ...
  // 为replicas选择DataNode,有两种BlockType
      ...
      synchronized (neededReconstruction) {
        for (int priority = 0; priority < blocksToReconstruct
          ...
          for (BlockInfo block : blocksToReconstruct.get(priority)) {
            // 找到block的源node
            //BlockReconstructionWork有两种，一种为ECWork,一种为普通Replicas
  ...
  public void processMisReplicatedBlocks() {
    ...
    // 初始化ReconstructQueue的线程，每隔一段时间就会扫描一遍Block,将那些Block副本数不够的Block放到ReconstructQueue中
  ...
  /**
   * 判断一个节点是否可以安全下线或maintenance,该节点下线之后不会影响Block的冗余
 ...
 public String getCorruptReason(Block block, DatanodeDescriptor node) {
    ...
    public void run() {
      while (namesystem.isRunning()) {
        try {
          ...
          if (isPopulatingReplQueues()) {
            // 处理Block的冗余问题,为DataNdoe分配block修复任务
            computeDatanodeWork();
  ...
  /**
   * 整理TreeSet的StorageInfo,并将他压缩，使其低于某一水平。
