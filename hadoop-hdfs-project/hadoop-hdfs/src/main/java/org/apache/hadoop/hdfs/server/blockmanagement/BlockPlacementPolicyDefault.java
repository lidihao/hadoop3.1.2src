...
public class BlockPlacementPolicyDefault extends BlockPlacementPolicy {
  ...
  DatanodeStorageInfo[] chooseTarget(String src,
      ...
      BlockStoragePolicy storagePolicy,// 为Replicas选择存储介质的策略
      EnumSet<AddBlockFlag> flags) {
    try {
  ...
  private DatanodeStorageInfo[] chooseTarget(int numOfReplicas,
    ...
    // 将已经选择的DataNode加入到excludedNodes中
   ...
   */
  protected DatanodeStorageInfo chooseRandom(int numOfReplicas,//副本数
                            String scope,
                            Set<Node> excludedNodes,//exclude的节点
                            long blocksize,//块大小
                            int maxNodesPerRack,//每一个Rack的最大节点数
                            List<DatanodeStorageInfo> results,//结果
                            boolean avoidStaleNodes,//跳开Stale节点
                            EnumMap<StorageType, Integer> storageTypes)// 存储类型:RAM_DISK(true),SSD(false),DISK(false),ARCHIVE(false),PROVIDED(false);
                            throws NotEnoughReplicasException {
    ...
    while (numOfReplicas > 0) {
      ...
      if (clusterMap instanceof DFSNetworkTopology) {
        // 存储介质，从fast到low,从网络拓扑中选择一个DataNode
        for (StorageType type : storageTypes.keySet()) {
    ...
    if (numOfReplicas>0) {
      ...
      /**
       * 2019-06-18 10:49:30,143 INFO org.apache.hadoop.hdfs.server.blockmanagement.BlockPlacementPolicy: Not enough replicas was chosen. Reason:{TOO_MANY_NODES_ON_RACK=5}
       * TOO_MANY_NODES_ON_RACK("the rack has too many chosen nodes"),
       */
